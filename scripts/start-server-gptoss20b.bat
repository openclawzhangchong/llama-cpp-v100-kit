@echo off
title gpt-oss-20b 启动器（选择上下文与思考强度）
rem ================================================
rem  gpt-oss-20b 交互式启动器
rem  第 1 步选上下文长度，第 2 步选思考强度
...
rem ---- 0. 检查模型文件是否存在 ----
set "MODEL=models\gpt-oss-20b-Q4_K_M.gguf"
if not exist "!MODEL!" (
    color 4F
    echo.
...
cls
echo ============================================
echo    第 1 步 / 共 2 步：选择上下文长度
echo ============================================
echo.
echo     1)  low      快速回答（日常闲聊、翻译）
echo     2)  medium   平衡模式（默认，推荐）
echo     3)  high     深度思考（数学/推理/复杂代码）
echo.
echo --------------------------------------------
...
echo    8K 约 13GB      32K 约 14GB      128K 约 23GB
echo --------------------------------------------
set "CTX="
set "KVQ="
set "TAG="
...
echo    停止服务  : 本窗口按 Ctrl+C，或双击 stopgptoss20b.bat
echo ============================================
echo.
:LAUNCH
...
llama-server.exe -m "!MODEL!" -ngl 99 -fa on -c !CTX! -ctk !KVQ! -ctv !KVQ! --chat-template-kwargs "{\"reasoning_effort\":\"%EFF%\"}" --path webui
